$(document).ready(function() {
    ...
    //Funcion que llama a los datos que estan registrados en mi base de datos.
    function fetchTasksContacto() {
        $.ajax({
            url: '../app/contacto-list.php',
            type: 'GET',
            success: function (response) {
                let tasks = JSON.parse(response);
                let template = '';
                tasks.forEach(task => {
                    template += `
                        <tr taskId="${task.id}">
                            ...
                            <td>${task.mensaje}</td>
                            <td>
                                <img src="../assets/icons/trash.svg" class="task-delete-contacto" value="Borrar">
                            </td>
                        </tr>
                    `
                });
                $('#tasksContacto').html(template);
            }
        });
    }

    //Funcion que me permite eliminar un contacto por su id sin necesidad de recargar la pagina.
    $(document).on('click', '.task-delete-contacto', function () {
        //Muestro un alert para confirmar eliminar el contacto.
        if (confirm('¿Quieres eliminar este contacto?')) {
            let element = $(this)[0].parentElement.parentElement;
            let id = $(element).attr('taskId');
            $.post('../app/contacto-delete.php', {id}, function (response) {
                fetchTasksContacto();
            });
        }
    });

    //Funcion que me permite editar un contacto sin necesidad de recargar la pagina.
    $(document).on('click', '.task-item', function () {
        let element = $(this)[0].parentElement.parentElement;
        let id = $(element).attr('taskId');
        $.post('../app/contacto-single.php', {id}, function (response) {
            const task = JSON.parse(response);
            $('#correo').val(task.correo);
            $('#mensaje').val(task.mensaje);
            $('#taskId').val(task.id);
            edit = true;
        })
    });
